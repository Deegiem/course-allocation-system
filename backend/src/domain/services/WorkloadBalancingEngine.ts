...
export class WorkloadBalancingEngine {
  ...
  calculateDistribution(
    ...
    courses: ICourse[]
  ): IWorkloadDistribution[] {
    const distribution: IWorkloadDistribution[] = [];
    const courseMap = new Map(courses.map(c => [c.id, c]));

    // Group allocations by lecturer
    const lecturerAllocations = new Map<number, IAllocation[]>();
  ...
  analyzeBalance(distribution: IWorkloadDistribution[]): IBalanceResult {
    ...
    const variances = activeLecturers.map(d =>
      Math.pow(d.utilization - averageUtilization, 2)
    );
  ...
  optimizeDistribution(
    distribution: IWorkloadDistribution[],
    unallocatedCourses: ICourse[]
  ): {
    assignments: { courseId: number; suggestedLecturerId: number; reason: string }[];
    ...
    const availableLecturers = sortedByUtilization.filter(d =>
      d.currentLoad < d.maxLoad
    );

    // Assign unallocated courses
    for (const course of unallocatedCourses) {
      // Find best lecturer (lowest utilization with capacity)
      const target = availableLecturers.find(d =>
        d.currentLoad + course.units <= d.maxLoad
      );
