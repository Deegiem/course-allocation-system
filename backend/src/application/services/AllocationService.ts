...
import { IAllocation, ISystemPolicy, DefaultSystemPolicy, ICourse } from '../../domain/entities';
import { AllocationStatus, OverrideAction } from '../../domain/enums';
import { BaseService } from './BaseService';

interface ScoredLecturer {
    lecturerId: number;
    score: number;
    name: string;
...
export class AllocationService extends BaseService implements IAllocationService {
    constructor(
    ...
    async allocateAutomatically(
        ...
        try {
            ...
            for (const course of courses) {
                ...
                const scoredLecturers = await this.scoreLecturersForCourse(
                    lecturers,
                    course,
                    policies,
                    workloads
    ...
    private async scoreLecturersForCourse(
        lecturers: any[],
        course: any,
        policies: ISystemPolicy,
        workloads: Map<number, number>
    ): Promise<ScoredLecturer[]> {
        const scored: ScoredLecturer[] = [];
        ...
        for (const lecturer of lecturers) {
            ...
            // Get weights
            const weights = policies.weights || DefaultSystemPolicy.weights;

            // Calculate total score
            const score =
                (specializationMatch * (weights.expertise || 0.40)) +
    ...
    private hasSpecializationMatch(lecturerSpecialization: any, courseSpecialization: string): boolean {
        if (!lecturerSpecialization) return false;

        // Convert both to arrays of trimmed lowercase strings for comparison
        let lecturerSpecs: string[] = [];
        let courseSpecs: string[] = [];
    ...
    private getCourseSpecialization(course: ICourse): string {
    ...
    async getAllocations(): Promise<IAllocation[]> {
        return await this.allocationRepository.findActiveAllocations();
    }

    async getLecturerAllocations(lecturerId: number): Promise<IAllocation[]> {
        return await this.allocationRepository.findByLecturer(lecturerId);
    }

    async getCourseAllocations(courseId: number): Promise<IAllocation[]> {
        return await this.allocationRepository.findByCourse(courseId);
    }

    async getPendingAllocations(): Promise<IAllocation[]> {
        return await this.allocationRepository.findPendingAllocations();
    }

    async validateAllocation(lecturerId: number, courseId: number): Promise<{
        valid: boolean;
        message?: string;
        conflicts?: string[];
    }> {
        ...
        const hasSpecializationMatch = (lecturerSpecialization: any, courseSpec: string): boolean => {
            if (!lecturerSpecialization) return false;
            if (typeof lecturerSpecialization === 'string') {
            ...
            if (Array.isArray(lecturerSpecialization)) {
                return lecturerSpecialization.some(spec =>
    ...
    async approveAllocation(allocationId: number, approvedBy: string): Promise<IAllocation> {
        try {
            const allocation = await this.allocationRepository.findById(allocationId);
            if (!allocation) {
                throw new Error(`Allocation with ID ${allocationId} not found`);
    ...
    async rejectAllocation(allocationId: number, reason: string, rejectedBy: string): Promise<IAllocation> {
        try {
            const allocation = await this.allocationRepository.findById(allocationId);
            if (!allocation) {
                throw new Error(`Allocation with ID ${allocationId} not found`);
